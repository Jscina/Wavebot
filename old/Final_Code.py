...
def calculate_eye_angles(x_val, y_val):
    ...
    if x_val < 0:  # If x_val is negative, the face is on the right side of the face
        lex_angle = 125 - angle_diff*sigmoid(angle_diff)-3 # Adjusted calculation for left eye
        rex_angle = 130 - angle_diff*sigmoid(angle_diff)-3 # Adjusted calculation for right eye
    ...
    ley_angle = (120 - (y_val * vertical_sensitivity_factor/1.5 * (120 - 300) / 240)) + vertical_angle_diff
    rey_angle = (110 + (y_val * vertical_sensitivity_factor/1.5 * (110 - 300) / 240)) + vertical_angle_diff +3
...
def update_neck_x_servo(x_val):
    ...
    neck_x_angle = max(30, min(100, neck_x_angle))  # Limit the neck x angle between 50 and 100
